...
class ConversationProvide with ChangeNotifier {
  ...
  /// 添加对话
  Future<void> addConversation(ConversationModel conversation, ChatMessageModel message) async {
    conversations.insert(0, conversation);
    String userId = message.userId;
    bool isMeMessage = ChatMessageProvide.curConversation.chatUserId == userId || userId == SharedPreferencesUtil.userInfo['userId'].toString();
    if (!isMeMessage) {
  ...
  updateConversation(ConversationModel model, ChatMessageModel message) async {
    ...
    message.createTime = message.createTime == -1 ? DateTime.now().millisecondsSinceEpoch : message.createTime;
    // 会话列表消息保留当前时间最新的消息
  ...
  bool checkConversation(String chatUserId) {
    bool isExit = false;
    for (int i = 0; i < conversations.length; i++) {
      if ((conversations[i].chatUserId == chatUserId) && (conversations[i].userId == SharedPreferencesUtil.userInfo['userId'])) {
        isExit = true;
        break;
      }
    }
  ...
  cleanMessageState(BuildContext context, String chatUserId) async {
    ...
    Provider.of<ChatMessageProvide>(context, listen: false).cleanMessageState(model.conversationId);
  }
  ...
  delConversation(BuildContext context, ConversationModel conversation) async {
    ...
    Provider.of<ChatMessageProvide>(context, listen: false).delChatMessages(conversation.conversationId);
    notifyListeners();
  }
