...
class ChatMessageProvide with ChangeNotifier {
  ...
  receivedMessage(ChatMessageModel message) async {
    ...
    bool isMeMessage = curConversation.chatUserId == userId || userId == SharedPreferencesUtil.userInfo['userId'].toString();
    message.isRead = isMeMessage ? yNStatusEnum['Yes'] : yNStatusEnum["No"];
    message.createTime = message.createTime;

    if (message.conversationId != null) {
      ChatMessageModel addChatMessage = await _messageController.addChatMessage(message);
      if (addChatMessage.id == null) {
        //如果插入数据库不成功，就不更新页面
        return;
      }
    }
    if (isMeMessage && message.conversationId == curConversation.conversationId) {
      curMessages.insert(0, message);
      _sortMessages();
      count = await _messageController.getChatMessageCount(message.conversationId); //获取消息数量
      notifyListeners();
    }
  }
  ...
  cleanMessageState(int conversationId) {
    ...
    if (conversationId != null) {
      _messageController.updateChatMessageRead(conversationId, yNStatusEnum["Yes"]); //标记消息已读状态
    }
  }
  ...
  getChatMessages(int conversationId) async {
    ...
    List<ChatMessageModel> messages = await _messageController.getChatMessages(conversationId, curMessages.length);
