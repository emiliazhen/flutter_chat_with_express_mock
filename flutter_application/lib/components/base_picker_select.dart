...
class BasePickerSelect extends StatefulWidget {
  BasePickerSelect({Key? key, this.value, this.items = const [], this.props = const {'key': 'KeyId', 'value': 'Value'}, this.onSelected, this.json = false, this.title = '选择'})
      : super(key: key);
  final dynamic value;
  final List items;
  final Map props;
...
class BasePickerSelectState extends State<BasePickerSelect> with TickerProviderStateMixin {
          ...
          children: <Widget>[
            Expanded(
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 30.sp,
                  color: item['disable'] == true ? Colors.grey : Color(0xff666666),
                ),
              ),
            ),
            Offstage(
