...
import 'package:flutter_application/pages/chat_video_view.dart';
...
import 'package:flutter_application/components/status_view.dart';
...
class ChatMessageWidget extends StatelessWidget {
  ...
  void _resendMessage() {
    ...
    Provider.of<WebSocketProvide>(context, listen: false).sendMessage(context, conversation, newMessage);
  }
  ...
  Widget _messageStateArea() {
    ...
    EdgeInsetsGeometry curMargin = isMe ? EdgeInsets.only(right: 15.w) : EdgeInsets.only(left: 15.w);
    if (state == chatLogStatusEnum["Sending"] && DateTime.now().millisecondsSinceEpoch - message.createTime >= 10000) {
      message.state = chatLogStatusEnum["Failed"];
      message.failReason = '发送超时，请重试！';
      Provider.of<ChatMessageProvide>(context, listen: false).updateChatMessage(message);
    }
  ...
  Widget get _messageMainArea => LimitedBox(
      maxWidth: 490.w,
      child: Container(
          margin: isMe ? EdgeInsets.only(right: 24.w) : EdgeInsets.only(left: 24.w),
  ...
  Widget _messageContentText() {
    ...
    List<Widget> resultItems = msgList.map((item) {
      if (EmojiUtil.codeReg.hasMatch(item) && EmojiUtil.emojiData[item] != null) {
        return Image.asset(
    ...
    return CustomPopupMenu(
      ...
      menuBuilder: () => GestureDetector(
          onTap: () {
            Clipboard.setData(ClipboardData(text: EmojiUtil.emojiCodeToCN(message.content)));
      ...
      child: LimitedBox(
        maxWidth: 490.w,
        child: Stack(alignment: isMe ? AlignmentDirectional.topEnd : AlignmentDirectional.topStart, children: <Widget>[
          Container(
            constraints: BoxConstraints(minHeight: 80.w),
            padding: EdgeInsets.all(20.w),
            decoration: BoxDecoration(borderRadius: BorderRadius.circular(5), color: isMe ? Color(0xff2294E2) : Colors.white),
            child: Wrap(
            ...
            MaterialPageRoute(
              builder: (BuildContext context) {
                return PhotoViewSimple(
                  imageProvider: NetworkImage(Config.fileDomain + message.content),
                );
              },
  ...
  /// 语音消息
  Widget get _messageContentAudio => Stack(alignment: isMe ? AlignmentDirectional.topEnd : AlignmentDirectional.topStart, children: <Widget>[
        GestureDetector(
          onTap: () {
            if (audioTap != null) {
              audioTap!();
              ...
              child: Row(
                children: [
                  Container(
                      ...
                      child: Icon(
                        curPlayMessageId == message.msgId ? Icons.pause : Icons.play_arrow,
                        color: isMe ? Colors.white : Color(0xff6c98b7),
                      )),
                  message.duration > 0
                      ? Text(
                          '${message.duration.toString()}″',
                          style: TextStyle(color: isMe ? Colors.white : Color(0xff6c98b7)),
                        )
                      : SizedBox(
                          width: 25.w,
                          height: 25.w,
                          child: CircularProgressIndicator(
                            valueColor: AlwaysStoppedAnimation<Color>(isMe ? Colors.white : Color(0xff6c98b7)),
                            strokeWidth: 2.w,
                          ),
                        ),
                ],
              ...
              child: Container(
                ...
                child: Row(
                  children: <Widget>[
                    ...
                    Text(
                      '视频',
                      style: TextStyle(fontSize: 24.sp, color: isMe ? Colors.white : Color(0xff6c98b7)),
                    ),
                    Container(
        ...
        child: Container(
          ...
          child: Transform(
            ...
            child: Container(
              width: 16.w,
              height: 16.w,
              decoration: BoxDecoration(borderRadius: BorderRadius.circular(1), color: isMe ? Color(0xff2294E2) : Colors.white),
            ),
          ),
        ),
      );
