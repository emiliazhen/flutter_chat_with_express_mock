...
import 'package:flutter_application/service/base/index.dart';
...
class ConversationController {
  ...
  static const String CONVERSATION_TABLE_SQL = "CREATE TABLE  If NOT EXISTS  $_tableName" +
  ...
  dynamic _db;
  //建立索引 加快查询速度
  static const String CONVERSATION_INDEX_SQL = "CREATE INDEX  index_conversation ON $_tableName (conversationId,userId)";
  ...
  Future<ConversationModel> getConversationById(int id) async {
    try {
      await _init();
      List<Map<String, dynamic>> _query = await _db.query(_tableName, columns: ConversationModel().getColumns(), where: 'id = ?', whereArgs: [id]);
      if (_query.length > 0) {
  ...
  Future<ConversationModel> getConversationByUserId(String chatUserId) async {
    try {
      ...
      List<Map<String, dynamic>> _query =
          await _db.query(_tableName, columns: ConversationModel().getColumns(), where: 'chatUserId=? and userId=?', whereArgs: [chatUserId, _userId]);
      if (_query.length > 0) {
        return ConversationModel.toModel(_query.first);
      }
      return ConversationModel(); // 查询数据集为空 则返回空实体对象
  ...
  Future<ConversationModel> addConversation(ConversationModel model) async {
    try {
      ...
      _db.transaction((txn) async {
        List<Map<String, dynamic>> _query =
            await txn.query(_tableName, columns: ConversationModel().getColumns(), where: 'chatUserId=? and userId=?', whereArgs: [model.chatUserId, _userId]);
        if (_query.length > 0) {
  ...
  void updateConversation(ConversationModel model) async {
    try {
      ...
      Future.delayed(Duration(milliseconds: 100), () async {
        int _count = await _db.update(_tableName, model.toMap(), where: 'conversationId = ? and userId= ?', whereArgs: [model.conversationId, _userId]);
  ...
  void updateMemberInfoConversation(ConversationModel model) async {
    try {
      await _init();
      Future.delayed(Duration(milliseconds: 100), () async {
        int _count = await _db.update(_tableName, model.toMemberInfoMap(), where: 'conversationId = ? and userId = ?', whereArgs: [model.conversationId, _userId]);
  ...
  Future<int> deleteConversation(int conversationId) async {
    try {
      await _init();
      int _count = await _db.delete(_tableName, where: "conversationId = ? and userId=?", whereArgs: [conversationId, _userId]);
  ...
  Future<List<ConversationModel>> searchConversation(String key) async {
    await _init();
    List<ConversationModel> _result = [];
    List<Map<String, dynamic>> _querys = await _db.query(_tableName, columns: [], where: 'userId = ? and (trueName LIKE  ?)', whereArgs: [_userId, '%$key%']);
    if (_querys == null) {
      return _result;
    }
    _querys.forEach((q) {
