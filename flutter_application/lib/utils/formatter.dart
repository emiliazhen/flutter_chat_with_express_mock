...
class FormatterUtil {
  ...
  /// 显示的时间格式化; 精确到时分秒
  static String filterDateTime(String value, {String formatter = 'yyyy-MM-dd HH:mm:ss'}) {
    if (value.isEmpty) {
      return '-';
    }
    ...
    if (year == DateTime.now().year.toString()) {
      return DateFormat(formatter.replaceAll('yyyy-', '')).format(DateTime.parse(value));
    }
    return DateFormat(formatter).format(DateTime.parse(value));
  }

  /// 显示的时间格式化; 精确到时分
  static String filterDateMinutes(value, {String formatter = 'yyyy-MM-dd HH:mm'}) {
    if (value.isEmpty) {
      return '-';
    }
    String year = DateFormat('yyyy').format(DateTime.parse(value));
  ...
  /// rate
  /// showminus不显示负数
  static String filterFloat(value, {int fixed = 2, int rate = 10000, String prefix = '', String postfix = '', bool showminus = true}) {
  ...
  /// rate
  /// showminus不显示负数
  static String filterFloatRead(value, {int fixed = 2, int rate = 10000, String prefix = '', String postfix = '', bool showminus = true}) {
    if (value == null || value == '' || (!showminus && value < 0)) {
      value = 0;
    }
    if (value is String) {
  ...
  static String filtersMsgTime(int lastMsgTime) {
    ...
    String dateOriginal = formatter.format(DateTime.fromMillisecondsSinceEpoch(lastMsgTime));
