...
class MemberDetailPage extends StatefulWidget {
  ...
  MemberDetailPage({Key? key, required this.memberId, required this.memberName}) : super(key: key);
...
class _MemberDetailPageState extends State<MemberDetailPage> {
  ...
  /// 跳转聊天
  void _goChatDetail() {
    ConversationModel conversation = ConversationModel.toConversation(_info, ChatMessageModel());
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: _isLoading
          ? BaseLoading()
          : Column(
              children: <Widget>[_userPageTop, _descriptionTitle, Expanded(child: _descriptionContent)],
            ),
    );
  }
        ...
        children: [
          Container(
            ...
            decoration: const BoxDecoration(
                gradient: LinearGradient(colors: [Color.fromARGB(255, 167, 220, 245), Colors.blueGrey], begin: Alignment.topCenter, end: Alignment.bottomCenter)),
            child: Column(
  ...
  Widget _statisticsItem(String title, int total, {bool hasBorder = false}) {
    Widget child = Column(
      children: [
        ...
        Text(
          title,
          style: TextStyle(fontSize: 24.sp, color: Color.fromARGB(255, 209, 206, 206)),
        )
      ],
    );
    return hasBorder
        ? Container(
            ...
            decoration: BoxDecoration(
                border: Border(left: BorderSide(width: 1.w, color: Color.fromARGB(255, 209, 206, 206)), right: BorderSide(width: 1.w, color: Color.fromARGB(255, 209, 206, 206)))),
            child: child)
        : child;
  }
  ...
  Widget get _descriptionTitle => Container(
        ...
        child: Text('简介', style: TextStyle(fontSize: 36.sp, color: Colors.black87)),
      );
