...
import 'package:flutter_application/pages/member_detail.dart';
...
final List<String> memberIndexList = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'];
  ...
  void _getData() {
    ...
    apiUserList(context, data: _queryForm).then((res) {
      ...
      if (res.data['code'] == 200) {
        ...
        Provider.of<UserListProvide>(context, listen: false).setUserList(resData);
        setState(() {
        ...
        children: <Widget>[
          ...
          Expanded(
              ...
              child: Consumer<UserListProvide>(builder: (_, userList, child) {
                ...
                resList.removeWhere((item) => item['userId'] == SharedPreferencesUtil.userInfo['userId']);
  ...
  Widget get _searchBar => Container(
      ...
      child: GestureDetector(
          ...
          child: Container(
            ...
            height: 70.h,
            decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(10.sp)),
            child: Row(
...
class MemberSearchDelegate extends SearchDelegate {
  ...
  Widget get _resultListView => Consumer<UserListProvide>(builder: (_, userList, child) {
        ...
        final resList = query.isEmpty ? [] : nameList.where((input) => input['name'].contains(query)).toList();
        return ListView.builder(
            itemCount: resList.length,
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                title: queryIndex < 0
                    ...
                    : RichText(
                        text: TextSpan(text: userName.substring(0, queryIndex), style: TextStyle(color: Colors.grey, fontSize: 30.sp), children: [
                        TextSpan(text: userName.substring(queryIndex, queryIndex + query.length), style: TextStyle(color: Colors.blueAccent, fontWeight: FontWeight.bold)),
                        TextSpan(text: userName.substring(queryIndex + query.length)),
                      ])),
              );
            });
      });
