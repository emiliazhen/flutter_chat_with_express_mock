...
class _FindPageState extends State<FindPage> with AutomaticKeepAliveClientMixin {
  ...
  void _openFileClick(Map<String, dynamic> item) {
    OpenFile.open('$_sdCardPath/${item["fileName"]}.pdf', type: 'application/pdf');
  }
  ...
  void _downloadClick(Map<String, dynamic> item) async {
    ...
    setState(() {
      ...
      _progress = 0;
      _downloadingIndex = _data.indexWhere((dataItem) => dataItem['id'] == item['id']);
      _data[_downloadingIndex]['isDownloading'] = true;
    });
    try {
      await Dio().download('${Config.baseUrl}/download/file?fileId=${item['fileId']}', path, data: {'fileId': item['fileId']}, onReceiveProgress: (int count, int total) {
        if (count == -1) {
                ...
                child: Stack(
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      alignment: Alignment.center,
                      child: Text(
                        _progress == 0 ? '下载中..' : '${_progress.toStringAsFixed(2)}%',
